...
def preprocess_data(con, census_place_table_name: str, usa_state_geom_table: str, industry_table_name: str, path_sql_functions: str) -> None:
    logger.debug("Preprocessing data")

    logger.debug("Configuring database")
    _configure_db(con=con)

    logger.debug("Refactoring census_place table")
    _refactor_census_place_table(con=con, census_place_table_name=census_place_table_name)

    logger.debug("Refactoring usa_state_geom table")
    _refactor_usa_state_geom_table(con=con, usa_state_geom_table=usa_state_geom_table)

    logger.debug("Refactoring industry table")
    _refactor_industry_table(con=con, industry_table_name=industry_table_name)

    logger.debug("Inserting SQL functions")
    _insert_sql_functions(con=con, path_sql_functions=path_sql_functions)


def _configure_db(con):
    ...
    _execute_sql(con=con, query=query)


def _refactor_census_place_table(con, census_place_table_name: str) -> None:
...
def _refactor_usa_state_geom_table(con, usa_state_geom_table: str) -> None:
...
def _refactor_industry_table(con, industry_table_name: str) -> None:
    query = f"ALTER TABLE {industry_table_name} ADD PRIMARY KEY (code);"
    _execute_sql(con=con, query=query)


def _insert_sql_functions(con, path_sql_functions: str):
...
def create_data_table(con, geo_table_name: str, dem_table_name: str, data_table_name: str, census_place_table_name: str, industry_table_name: str) -> None:
    ...
    _transform_histid_geo_table_to_uppercase(con=con, geo_table_name=geo_table_name)

    logger.debug(f"Creating indices for {geo_table_name} and {dem_table_name}")
    _create_indices_geo_and_dem_tables(con=con, geo_table_name=geo_table_name, dem_table_name=dem_table_name)

    logger.debug(f"Removing duplicate histids from {geo_table_name} and {dem_table_name}")
    _remove_duplicate_hist_ids_from_geo_and_dem_tables(con=con, geo_table_name=geo_table_name, dem_table_name=dem_table_name)

    logger.debug(f"Merging {geo_table_name} and {dem_table_name} to create {data_table_name}")
    _merge_geo_and_dem_tables_to_create_data_table(con=con, geo_table_name=geo_table_name, dem_table_name=dem_table_name, data_table_name=data_table_name)

    logger.debug(f"Adding primary and foreign keys to {data_table_name}")
    _add_primary_and_foreign_keys_to_data_table(con=con, data_table_name=data_table_name, census_place_table_name=census_place_table_name, industry_table_name=industry_table_name)

    logger.debug(f"Dropping {geo_table_name} and {dem_table_name}")
    _drop_geo_and_dem_tables(con=con, dem_table_name=dem_table_name, geo_table_name=geo_table_name)


def _transform_histid_geo_table_to_uppercase(con, geo_table_name: str):
...
def _create_indices_geo_and_dem_tables(con, geo_table_name: str, dem_table_name: str):
    query = (f"CREATE INDEX ON {geo_table_name} (histid);"
             f"CREATE INDEX ON {dem_table_name} (histid);")
    _execute_sql(con=con, query=query)


def _remove_duplicate_hist_ids_from_geo_and_dem_tables(con, geo_table_name: str, dem_table_name: str):
    table_names = [geo_table_name, dem_table_name]
    for tab_name in table_names:
        _remove_duplicate_histid_from_table(con=con, tab_name=tab_name)


def _remove_duplicate_histid_from_table(con, tab_name: str):
...
def _merge_geo_and_dem_tables_to_create_data_table(con, geo_table_name: str, dem_table_name: str, data_table_name: str):
...
def _add_primary_and_foreign_keys_to_data_table(con, data_table_name: str, census_place_table_name: str, industry_table_name: str):
...
def _drop_geo_and_dem_tables(con, dem_table_name: str, geo_table_name: str):
...
# Step 1: create_clusters
def create_clusters(con, data_table_name: str, rasterized_census_place_table_name: str, cluster_table_name: str, convolved_raster_table_name: str, convolution_kernel_size: int, convolution_kernel_decay_rate: float, pixel_threshold: float, dbscan_eps: float, dbscan_min_points: int) -> None:
    ...
    _rasterize_census_places(con=con, data_table_name=data_table_name, rasterized_census_place_table_name=rasterized_census_place_table_name)

    logger.debug(f"Convolving {rasterized_census_place_table_name} to {convolved_raster_table_name}")
    _convolve_raster(con=con, rasterized_census_place_table_name=rasterized_census_place_table_name, convolved_raster_table_name=convolved_raster_table_name, convolution_kernel_size=convolution_kernel_size, convolution_kernel_decay_rate=convolution_kernel_decay_rate)

    logger.debug(f"Creating clusters from {convolved_raster_table_name}")
    _create_clusters_from_raster(con=con, convolved_raster_table_name=convolved_raster_table_name, cluster_table_name=cluster_table_name, pixel_threshold=pixel_threshold, dbscan_eps=dbscan_eps, dbscan_min_points=dbscan_min_points)


def _rasterize_census_places(con, data_table_name: str, rasterized_census_place_table_name: str) -> None:
...
def _convolve_raster(con, rasterized_census_place_table_name: str, convolved_raster_table_name: str, convolution_kernel_size: int, convolution_kernel_decay_rate: float) -> None:
...
def _create_clusters_from_raster(con, convolved_raster_table_name: str, cluster_table_name: str, pixel_threshold: float, dbscan_eps: float, dbscan_min_points: int) -> None:
...
# Step 2: create_cluster_industry_table
...
def create_cluster_data_tables(con, data_table_name: str, cluster_table_name: str, cluster_industry_table_name: str, industry_table_name: str) -> None:
    ...
    _create_cluster_industry_table(con=con, data_table_name=data_table_name, cluster_table_name=cluster_table_name, cluster_industry_table_name=cluster_industry_table_name)

    logger.debug(f"Adding population to {cluster_table_name}")
    _create_cluster_cluster_table_with_population(con=con, cluster_table_name=cluster_table_name, cluster_industry_table_name=cluster_industry_table_name)

    logger.debug(f"Adding primary and foreign keys to {cluster_industry_table_name}")
    _add_primary_and_foreign_keys_to_cluster_industry_table(con=con, cluster_table_name=cluster_table_name, cluster_industry_table_name=cluster_industry_table_name, industry_table_name=industry_table_name)


def _create_cluster_industry_table(con, data_table_name: str, cluster_table_name: str, cluster_industry_table_name: str) -> None:
...
def _create_cluster_cluster_table_with_population(con, cluster_table_name: str, cluster_industry_table_name: str) -> None:
...
def _add_primary_and_foreign_keys_to_cluster_industry_table(con, cluster_table_name: str, cluster_industry_table_name: str, industry_table_name: str) -> None:
...
def run_pipeline(con, steps: List[int], year: int, convolution_kernel_size: int, convolution_kernel_decay_rate: float, pixel_threshold: float, dbscan_eps: float, dbscan_min_points: int):
    ...
    path_sql_functions = '/Users/andrea/Desktop/PhD/Data/ipums/sql'
    ...
    if 0 in steps:
        preprocess_data(con=con, census_place_table_name=census_place_table_name, usa_state_geom_table=usa_state_geom_table, path_sql_functions=path_sql_functions, industry_table_name=industry_table_name)
    logger.info(f"Creating data table for year {year}")
    if 1 in steps:
        create_data_table(con=con, geo_table_name=geo_table_name, dem_table_name=demographic_table_name, data_table_name=data_table_name, census_place_table_name=census_place_table_name, industry_table_name=industry_table_name)
    logger.info(f"Creating clusters for year {year}")
    if 2 in steps:
        create_clusters(con=con, data_table_name=data_table_name, rasterized_census_place_table_name=rasterized_census_place_table_name, convolved_raster_table_name=convolved_raster_table_name, convolution_kernel_size=convolution_kernel_size,
                        convolution_kernel_decay_rate=convolution_kernel_decay_rate, cluster_table_name=cluster_table_name, pixel_threshold=pixel_threshold, dbscan_eps=dbscan_eps, dbscan_min_points=dbscan_min_points)
    logger.info(f"Creating cluster industry table for year {year}")
    if 3 in steps:
        create_cluster_data_tables(con=con, data_table_name=data_table_name, cluster_table_name=cluster_table_name, cluster_industry_table_name=cluster_industry_table_name, industry_table_name=industry_table_name)
    logger.info(f"Pipeline for year {year} completed")


def _execute_sql(con, query: str):
...
if __name__ == '__main__':
    ...
    run_pipeline(con=conn, steps=[3], year=1930, convolution_kernel_size=11, convolution_kernel_decay_rate=0.2, pixel_threshold=100, dbscan_eps=100, dbscan_min_points=1)
